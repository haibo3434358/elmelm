...
@section('content')
    ...
    <div class="mws-panel grid_8">
        <div class="mws-panel-header">
            <span><i class="icon-users"></i>友情链接列表</span>
        </div>
        <div class="mws-panel-body no-padding">
            <div id="DataTables_Table_1_wrapper" class="dataTables_wrapper" role="grid">
                <form action="{{url('admins/youqing')}}" method="get">
                ...
                <table aria-describedby="DataTables_Table_1_info" id="DataTables_Table_1"
                    ...
                    @foreach($res as $k=>$v)
                        <tr class="@if ($k % 2 == 1) odd @else even @endif">
                            <td class="  sorting_1">{{$v->lid}}</td>
                            <td class=" ">{{$v->link_name}}</td>
                            <td class=" ">{{$v->l_url}}</td>
                            <td class=" " style="align:center">
                                <a  class="btn btn-info" href="{{url('admins/youqing/'.$v->lid.'/edit')}}">修改</a>
                                <a  class="btn btn-danger" href="javascript:void(0)" onclick="delUser({{$v->lid}})">删除</a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
...
//            layer.msg('删除成功', {icon: 1});
                            $.post("{{url('admins/youqing/')}}/"+id,{'_method':'delete','_token':'{{csrf_token()}}'},function(data){
//                console.log(data);
                                if(data.status == 0){
