...
//切换表单
    ...
    $('input[name=uname]').blur(function(){
        ...
        if(!reg.test(tv)){
            ...
            $.post("{{url('home/login/dyanzhenguname')}}",{'uname':tv,'_token':'{{csrf_token()}}'},function(data){
...
//                console.log(data);
    ...
    $('input[name=duname]').blur(function(){
        ...
        if(!reg.test(av)){
            ...
            //发送ajax
            {{--$.post("{{url('admin/link/')}}/"+id,{'_method':'delete','_token':'{{csrf_token()}}'},function(data)--}}
            $.post("{{url('home/login/yanzhenguname')}}",{'duname':av,'_token':'{{csrf_token()}}'},function(data){
...
//                console.log(data);
                if(data == 0){
                    ...
                    $('#duname').text(' √用户已注册').css('color','green');
